...
/**
 * Due to the way GameNetworkingSockets works it isn't easy to run two applications at the same
 * time. I'll try and figure out a better way to test connection in the future. For now, I will
 * leave the code here before I realised that the tests don't work.
 */
...
//             ++*frames;
//             if (application.Get_Network().Get_Network_State() == Network::Running)
//             server_ran->store(true);
//         });
...
//         [](chrono::milliseconds delta_time, ApplicationWindow& window) {},
//         [server_frames, client_connected](chrono::milliseconds delta_time, Application&
//         application) {
...
//         [](chrono::milliseconds delta_time, ApplicationWindow& window) {},
//         [client_frames, client_connected](chrono::milliseconds delta_time, Application&
//         application) {
//             if (*client_frames == 0) application.Start_Client();
//             if (*client_frames == 10 || *client_connected) application.Close_Application();
//             ++*client_frames;
//             if (application.Get_Network().Get_Network_State() == Network::Connected)
