...
float EUI_Element::Get_Font_Spacing(const EUI_Context& ctx) const {
    if (style.font_spacing.has_value())
...
Alignment EUI_Element::Get_Horizontal_Alignment(const EUI_Context& ctx) const {
    if (style.horizontal_alignment.has_value())
        return style.horizontal_alignment.value();
    if (parent)
...
Alignment EUI_Element::Get_Vertical_Alignment(const EUI_Context& ctx) const {
    if (style.vertical_alignment.has_value())
        return style.vertical_alignment.value();
    if (parent)
...
Alignment EUI_Element::Get_Text_Horizontal_Alignment(const EUI_Context& ctx) const {
    if (style.text_horizontal_alignment.has_value())
        return style.text_horizontal_alignment.value();
    if (parent)
        return parent->Get_Text_Horizontal_Alignment(ctx);
    return ctx.default_style.text_horizontal_alignment.value();
}

Alignment EUI_Element::Get_Text_Vertical_Alignment(const EUI_Context& ctx) const {
    if (style.text_vertical_alignment.has_value())
        return style.text_vertical_alignment.value();
    if (parent)
        return parent->Get_Text_Vertical_Alignment(ctx);
    return ctx.default_style.text_vertical_alignment.value();
}
...
EUI_Style EUI_Element::Get_Effective_Style(const EUI_Context& ctx) const {
    ...
    if (style.text_horizontal_alignment.has_value())
        ...
        effective.text_horizontal_alignment = parent->Get_Text_Horizontal_Alignment(ctx);
    else
        effective.text_horizontal_alignment = ctx.default_style.text_horizontal_alignment;

    if (style.text_vertical_alignment.has_value())
        effective.text_vertical_alignment = style.text_vertical_alignment;
    else if (parent)
        effective.text_vertical_alignment = parent->Get_Text_Vertical_Alignment(ctx);
    else
        effective.text_vertical_alignment = ctx.default_style.text_vertical_alignment;
